...
public class Shop {
    public static void run() {
        ...
        Matcher matcher=getCommandMatcher(input, "shop buy ([A-Za-z]+)");
...
////            System.out.println(((MonsterCard)Card.getAllCards().get(0)).getAttack());
...
//        }
    ...
    public static boolean checkEnoughMoney(String cardName) {
            ...
            if (Card.getCardByName(cardName) != null) {
                Player player = LoginMenu.getLoggedInPlayer();
                if (Card.getCardByName(cardName).getPrice() > player.getMoney()) System.out.println("not enough money");
                return false;
            }
